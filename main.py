...
async def create_course(create_course_request: CreateCourseRequest, db: Session = Depends(get_db)) -> int:
    ...
    if instructor is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"No instructor found with id {create_course_request.instructor_id}")
...
async def add_student_to_course(course_id: int, request: AddStudentToCourseRequest, db: Session = Depends(get_db)) -> None:
    ...
    if student is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Student with ID {request.student_id} not found")
    course: Course | None = db.get(Course, course_id)
    if course is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Course with ID {course_id} not found")
